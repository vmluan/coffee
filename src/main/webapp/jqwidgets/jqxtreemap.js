	...
	d["default"] = (function() {
		function g(r, s, q, p) {
			...
			this.getCoordinates = function(y) {
				var x = [], v = this.xoffset, z = this.yoffset, w = j(y)
						/ this.height, u = j(y) / this.width;
			...
			this.cutArea = function(w) {
				var y;
				if (this.width >= this.height) {
					var x = w / this.height, t = this.width - x;
					y = new g(this.xoffset + x, this.yoffset, t, this.height)
				} else {
					var v = w / this.width, u = this.height - v;
		...
		function e(s, q, x, u, p) {
			...
			if (h(s[0])) {
				...
				w = m(v, q, x, u, p);
				for ( var r = 0; r < s.length; r += 1) {
					t.push(e(s[r], w[r][2] - w[r][0], w[r][3] - w[r][1],
							w[r][0], w[r][1]))
				}
			} else {
				t = m(s, q, x, u, p)
		...
		function k(t, s) {
			var q = Math.min.apply(Math, t), p = Math.max.apply(Math, t), r = j(t);
			return Math.max(Math.pow(s, 2) * p / Math.pow(r, 2), Math.pow(r, 2)
					/ (Math.pow(s, 2) * q))
		}
		function h(p) {
			...
			.extend(
					c.jqx._jqxTreeMap.prototype,
					{
						...
						render : function() {
							...
							this._destroy();
							this._root = new b(undefined, 0, null, [],
									this.host);
							var e = function(h, k) {
								var n = {}, l;
								var m = null;
							...
							if (c.jqx.dataAdapter && this.source != null
									&& this.source._source) {
						...
						dataBind : function(e, m) {
							...
							default:
								if (e.localdata != null) {
									...
									f.dataBind();
									j(this);
									f.bindBindingUpdate(this.element.id,
											function(n) {
												j(k, n)
											})
								}
						...
						_colorEvaluator : {
							...
							_toHex : function(f) {
								var i = f.r.toString(16), h = f.g.toString(16), e = f.b
										.toString(16);
								i = i.length === 1 ? "0" + i : i;
								h = h.length === 1 ? "0" + h : h;
								e = e.length === 1 ? "0" + e : e;
							...
							_isRgb : function(e) {
								return (/(rgb|rgba)\s*\(\s*\d+\s*(,\s*\d+\s*){2}(,\d+\.\d+)?\)(;?)/i)
										.test(e)
							},
							_isHex : function(e) {
								return (/^(#([0-9A-F]{3})([0-9A-F]{3})?)$/i)
										.test(e)
							},
							getColorByValue : function(n, j, k) {
								var o = this._colorEvaluator, m, l, e, f, g, j;
								if (o._isRgb(k)) {
									k = o._toHex(k)
								}
								k = o._toRgb(k);
							...
							parent : function(g) {
								var f = g.parent.color, e = this._colorEvaluator;
								if (!g.parent) {
									return "#fff"
								}
								if (!f) {
								...
								f = e.getColorByValue.call(this, g.value,
										g.parent.children, f);
								g.color = f;
								return f
							},
							autoColors : function(g) {
								var f = this.baseColor, e = this._colorEvaluator;
								f = e.getColorByValue.call(this, g.value,
										this._dataList, f);
								g.color = f;
								return f
						...
						_getColor : function(f) {
							var e = f.color, g = this.colorMode;
							if (this._isColor(e)) {
								return e
							}
						...
						_renderRect : function(i, f) {
							...
							j.addClass(this
									.toThemeProperty("jqx-treemap-rectangle"));
						...
						_centerLabel : function(g, f) {
							...
							if (f === a.HORIZONTAL || f === a.BOTH) {
								e.style.left = (g[0].offsetWidth - e.offsetWidth)
										/ 2 + "px"
							}
							if (f === a.VERTICAL || f === a.BOTH) {
								e.style.top = (g[0].offsetHeight - e.offsetHeight)
										/ 2 + "px"
							}
						},
									...
									.bind(
											...
											function(h) {
												if (f.hoverEnabled) {
													...
													e
															.addClass(f
																	.toThemeProperty("jqx-treemap-rectangle-hover"))
												}
												f._trigger("mouseenterSector",
														g)
											});
							e
									.bind(
											...
											function(h) {
												if (f.hoverEnabled) {
													e
															.removeClass("jqx-treemap-rectangle-hover")
												}
												f._trigger("mouseleaveSector",
														g)
											});
							e
									.bind(
						...
						clearSelection : function() {
							this.host
									.find(".jqx-treemap-rectangle-hover")
									.removeClass(
											this
													.toThemeProperty("jqx-treemap-rectangle-hover"));
						...
						_layoutArea : function(f, e) {
							if (f.children.length && f.children.length > 0) {
								...
								e
										.addClass(this
												.toThemeProperty("jqx-treemap-rectangle-parent"))
							} else {
						...
						_render : function(f, l) {
							...
							var n = this._getValues(f.children), g = f.area
									.offset(), j = l(n, f.area.width(), f.area
									.height()
									- e, 0, e), k, m;
						...
						_layout : function(f, l) {
							...
							var n = this._getValues(f.children), g = f.area
									.offset(), h = l(n, f.area.width(), f.area
									.height()
									- e, 0, e), m;
						...
						_layoutRect : function(i, f) {
							var j = f.area, e = i[2] - i[0], l = i[3] - i[1];
							j.css({
								left : i[0] - 1,
								top : i[1] - 1,
								width : e,
							...
							if (typeof this.renderCallbacks[f.label] === "function") {
								...
								var e = j.width() - 2;
								j.find(".jqx-treemap-label:first").css(
										"max-width", e)
							}
						},
						_getBoundValues : function() {
							var g = this._root, f = [], k, j = {}, e = {};
						...
						_getAutocolorRanges : function() {
							var l = this._getBoundValues(), g = 5, e = l[1].value, j = l[0].value, f = (e - j)
									/ g, m, k = [];
							for ( var h = j; h < e; h += f) {
								m = Math.round(h);
								k
										.push({
											min : m,
											max : h + f,
											color : this._colorEvaluator.getColorByValue
													.call(this, m,
															this._dataList,
															this.baseColor)
										})
						...
						_renderLegendLabel : function(f) {
							var g = c('<tr><td colspan="' + f.find("td").length
									/ 2 + '"/></tr>'), e = c('<div class="'
									+ this
											.toThemeProperty("jqx-treemap-legend-label")
									+ '" />');
						...
						_renderColorLegend : function(e) {
							var o = c('<div class="'
									+ this
											.toThemeProperty("jqx-treemap-legend")
									+ '"/>'), q, n, r, p = function(i) {
								return i
							};
							...
							var s = c('<table class="'
									+ this
											.toThemeProperty("jqx-treemap-legend-table")
									+ '"/>');
							o.append(s);
							s.append("<tr/>");
							o.append("<div/>");
							...
							n
									.addClass(this
											.toThemeProperty("jqx-treemap-legend-values"));
							var g = function(t, i) {
								try {
									if (t.min < i.min) {
										return -1
							...
							for ( var k = 0; k < e.length; k += 1) {
								var j = c('<td class="'
										+ this
												.toThemeProperty("jqx-treemap-legend-color")
										+ '"/>');
								...
								if (k === 0) {
									var l = c('<span class="'
											+ this
													.toThemeProperty("jqx-treemap-legend-max-value")
											+ " "
											+ this
													.toThemeProperty("jqx-treemap-legend-value")
											+ '"/>');
									l.text(p(e[k].min));
									n.append(l);
							...
							o.css({
								...
								visibility : (this.showLegend) ? "visible"
										: "hidden"
							});
							return o
						},
							...
							while (e.length) {
								...
								for ( var h = 0; h < k.length; h += 1) {
									...
									if (l.parent === g.label
											|| (!l.parent && !g.label)) {
										var j = g;
										m = new b(l.label, parseFloat(l.value,
												10), j, [], null, l.color,
												l.data, l.record);
										g.children.push(m);
										e.push(m)
						...
						propertyChangedHandler : function(g, e, h) {
							...
							if ((/hoverEnabled|selectionEnabled/).test(e)) {
								...
								if (e === "showLegend") {
									this.host.find("jqx-treemap-legend")
											.toggle()
								} else {
									this._refresh()
								}
						...
						//luan: add new function here
						//to call this function: $('#treemap').jqxTreeMap('updateData', data);
